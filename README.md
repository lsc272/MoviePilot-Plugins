...
- 热门 TMDB 片单：支持热门/高分榜单，以及奥斯卡、金球奖、IMDb、豆瓣 Top 250、Letterboxd、Criterion 等公开片单；目录使用来源页面实际标题，并可打开原始链接。
...
- 匹配预览：同步前逐项显示海报、片名、年份、类型、TMDB ID、Emby 匹配和缺失原因；已同步合集也可随时重新预览来源。
...
- 合集海报：使用多张影视海报生成倾斜海报墙，底部浅色遮罩向上渐隐；标题使用内置 Noto Sans CJK Medium 字体、适度字距和自适应排版，不再显示额外英文副标题。也可随时重新生成或上传自定义图片，旧版自动海报会在下次同步时升级。

...
- 合集管理：记录由插件管理的合集，支持单个或定时批量重新同步，也可同时删除 Emby 合集。
- 其他合集工具：识别未由智能合集管理的 Emby 合集，支持本地 ZIP 备份、合并恢复，以及强制自动备份后的批量清理；备份包含成员、元数据和压缩主海报并保留最近五份。
- 合集元数据：预览展示来源简介，同步时自动将简介写入 Emby 合集。
- 合集海报：使用多张影视海报生成倾斜海报墙，黑色遮罩从底部向上渐隐；标题采用暖金渐变、居中大字和自适应排版。也可随时重新生成或上传自定义图片，旧版自动海报会在下次同步时升级。
